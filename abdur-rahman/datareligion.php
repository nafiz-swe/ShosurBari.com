...
<head>
  ...
  <title>Religion-Admin | ShosurBari</title>
</head>
<body>
  ...
  $sql = "SELECT * FROM 8bd_religion_details $limit OFFSET $start";
  $result = mysqli_query($conn, $sql);
    echo "<h1>ধর্মীয় বিষয়</h1>";
    echo '<div class="table-wrapper">';
      ...
      if (isset($_POST['search'])) {
        ...
        $sql = "SELECT * FROM 8bd_religion_details WHERE user_id = $searchUserId ORDER BY user_id DESC $limit";
        $result = mysqli_query($conn, $sql);
      } else {
        $sql = "SELECT * FROM 8bd_religion_details ORDER BY user_id DESC $limit OFFSET $start";
        $result = mysqli_query($conn, $sql);
      }
      ...
      echo '<table>';
        ...
        echo '<td><a target="_blank" href="edit_religion.php?id=' . $row['user_id'] . '">Edit</a></td>';
        echo '</tr>';
        }
      echo '</table>';
